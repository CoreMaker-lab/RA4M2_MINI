...
/* Actual period: 0.0007281777777777778 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x10000,
  .duty_cycle_counts = 0x8000, .source_div = (timer_source_div_t) 0, .channel = 6, .p_callback = gpt6_callback,
...
/* Actual period: 0.001 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x15f90,
  .duty_cycle_counts = 0xafc8, .source_div = (timer_source_div_t) 0, .channel = 3, .p_callback = NULL,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
